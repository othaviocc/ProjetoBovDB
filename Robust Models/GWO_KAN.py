...
# Padronização (Obrigatório para KAN)
...
def avaliar_kan_gwo(solution):
    """Função de Fitness para o GWO: Treina no Treino, testa na Validação"""
    # Decodificando a solução (GWO trabalha com arrays contínuos)
    ...
    epochs = 15 # Baixo para a otimização rodar rápido
    ...
    return acc # O Mealpy vai tentar maximizar este valor

print("\nIniciando otimização Grey Wolf Optimizer (GWO) com KAN...")
start_time = time.time()

# Configuração do problema para o Mealpy
problem_dict = {
    "obj_func": avaliar_kan_gwo,
    "bounds": {"lower": BOUNDS_LOW, "upper": BOUNDS_HIGH}, # Limites contínuos
    "minmax": "max", # Queremos a maior acurácia
    "log_to": None   # Desativa logs massivos no terminal
}

# Configurações equivalentes à sua Tabela 1 (Gerações e População)
EPOCHS = 3000 # Equivalente ao ngen
POP_SIZE = 20 # Equivalente ao pop
...
# Juntando treino e validação para o modelo final ter mais dados
...
final_model.train()
for epoch in range(100): # Treino profundo
    optimizer.zero_grad()
...
print("\n" + "="*50)
print("RELATÓRIO FINAL OOS - GWO + KAN")
print("="*50)
...
nome_arquivo_kan = 'gwo_kan_previsoes_features_5min.csv'
df_temporal_kan.to_csv(nome_arquivo_kan, index=False)
print(f"Arquivo temporal '{nome_arquivo_kan}' salvo com sucesso! Pronto para os gráficos do artigo.")