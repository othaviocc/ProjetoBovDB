...
# Padronização (Crucial para a estabilidade da rede KAN)
...
# Limites de busca: [Hidden Dim, Learning Rate, Grid Size]
BOUNDS_LOW =  [8,  0.0001, 3]
BOUNDS_HIGH = [64, 0.05,  10]

def avaliar_kan_ao(solution):
    """Função de Fitness para o AO: Treina na base de Treino, testa na Validação"""
    # Decodificando a solução contínua sugerida pela Águia
    ...
    epochs = 15 # Valor baixo para agilizar o ciclo da metaheurística
...
print("\nIniciando otimização Aquila Optimizer (AO) com KAN...")
start_time = time.time()

# Dicionário de configuração exigido pelo Mealpy
problem_dict = {
    "obj_func": avaliar_kan_ao,
    "bounds": {"lower": BOUNDS_LOW, "upper": BOUNDS_HIGH}, 
    "minmax": "max", # Foco em maximizar a acurácia de validação
    "log_to": None   
}

# Parâmetros padronizados (equivalentes à Tabela 1 original)
EPOCHS = 3000 
POP_SIZE = 20 
...
print("\nTreinando o modelo definitivo para validação Out-of-Sample...")
# Combinando bases de Treino e Validação para entregar o máximo de informação ao modelo final
...
final_model.train()
for epoch in range(100): # Treino definitivo com mais épocas
    optimizer.zero_grad()
...
print("\n" + "="*50)
print("RELATÓRIO FINAL OOS - AO + KAN")
print("="*50)
...
print("\nGerando CSV com a série temporal de features e previsões (KAN)...")
...
# Organizando as colunas para facilitar a plotagem no seu artigo
...
nome_arquivo_kan = 'ao_kan_previsoes_features_5min.csv'
df_temporal_kan.to_csv(nome_arquivo_kan, index=False)
print(f"Arquivo temporal '{nome_arquivo_kan}' gerado com sucesso!")