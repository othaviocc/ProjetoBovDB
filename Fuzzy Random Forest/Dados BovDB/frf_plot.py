...
class Fuzzifier:
    ...
    def fit(self, X):
        for col in X.columns:
            # Usa o "Mínimo" e "Máximo" ignorando o 1% de outliers bizarros
            min_val = np.percentile(X[col].dropna(), 1)
            max_val = np.percentile(X[col].dropna(), 99)
            
            # Se a coluna for uma linha reta (sem variância), usa o min/max absoluto
            if min_val == max_val: 
                min_val, max_val = X[col].min(), X[col].max()
                if min_val == max_val: max_val += 1e-9 
                
            # divido perfeitante igual
            if 'gmfmfs' in self.mf_type:
                self.params[col] = np.linspace(min_val, max_val, self.n_partitions + 1)
            else:
    ...
    def transform(self, X):
        ...
        for col in X.columns:
            ...
            if 'gmfmfs' in self.mf_type:
                ...
                centers = params
                #uso de linspace, então a distância é sempre perfeita e igual entre os centros
...
class FuzzyRandomForest:
    # Adicionado min_samples_leaf para manter controle dinâmico
    def __init__(self, n_estimators=100, mf_type='gaussian', n_partitions=3, voting='MWLT', max_depth=8, min_samples_leaf=10):
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_estimators):
            ...
            # Repassando os parâmetros dinâmicos em vez de hardcode (max_depth=8)
            fdt = FuzzyDecisionTree(max_depth=self.max_depth, min_samples_leaf=self.min_samples_leaf)
...
if __name__ == "__main__":
    ...
    for mf in mfs:
        for r in ramos_lista:
            for est in estrategias:
                # Agora o max_depth pode ser passado livremente aqui se quiser testar no Grid Search depois
                model = FuzzyRandomForest(mf_type=mf, voting=est, n_partitions=r, max_depth=8, min_samples_leaf=10)
    ...
    # CORREÇÃO 3: Decimals = 5 para revelar os cortes precisos e evitar "perguntas duplicadas"
    print(export_text(tree_top, feature_names=[f_names[f] for f in feats_top], max_depth=8, decimals=5))
